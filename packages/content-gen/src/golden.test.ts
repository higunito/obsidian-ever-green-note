...
/**
 * `reference/obsidian` は実 Vault の断面（読み取り専用）。現時点では Fleeting/・Collections/ が無く、
...
describe("generateContent（reference/obsidian ゴールデンテスト）", () => {
	...
	it("公開ゾーンに publish:true のノートが無いため、記事・グラフ・検索・コレクション・パスは空になる", () => {
		expect(result.articles).toEqual([]);
		expect(result.graph).toEqual({ topics: [], nodes: [], edges: [] });
		expect(result.searchIndex).toEqual({ items: [] });
		expect(result.collections).toEqual({ items: [] });
