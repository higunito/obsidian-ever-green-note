 ...
 * 設定は `localStorage` に保存し即時反映する（spec SC-011 §11.3）。利用不可時は既定値
 * （アニメーション低減 OFF・文字サイズ「大」）のまま動作する。
 * SSR とクライアント初期表示のずれを避けるため、マウント前は表示状態を各既定値で表示する
 * （layout.tsx の beforeInteractive スクリプトが実際の演出停止／文字サイズ自体は先に反映済みのため、
 * この表示ラグは演出・文字サイズのちらつきには影響しない）。
...
export function ConfigPanel() {
	...
	function toggleFontSize() {
		const next: FontSize = fontSize === "large" ? "small" : "large";
	...
	// 未マウント時は OS 設定（prefers-reduced-motion）／既定値（文字サイズ「大」）に委ね、属性を明示的に外す
